...
*)
	echo -e " 
	本 ${red}脚本${none} 不支持你的系统。 ${yellow}(-_-) ${none}
...
if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
	...
	echo -e " 出错了， ${red}请重新安装V2Ray${none} ${yellow}~(^_^) ${none}" && exit 1
fi
...
change_shadowsocks_port() {
	...
	while :; do
		...
		$v2ray_port)
			echo
			echo -e " 不能和$cyan V2Ray 端口 $none相同"
			error
			;;
		[1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
			if [[ $v2ray_transport == [45] ]]; then
...
disable_shadowsocks() {
	...
	while :; do
		...
		if [[ "$y_n" == [Yy] ]]; then
			...
			echo
			echo
			echo -e "$green Shadowsocks 已关闭，也可以随时重新启用 Shadowsocks$none"
			echo
			break
		elif [[ "$y_n" == [Nn] ]]; then
			echo
...
change_v2ray_port() {
	if [[ $v2ray_transport == 4 ]]; then
		echo
		echo -e " 目前使用的是$yellow WebSocket + TLS $none传输协议，不需要修改"
		echo
		...
		echo
		echo -e " 由于你目前使用的是$yellow HTTP/2 $none传输协议，不需要修改"
		echo
		echo " 如果你想要使用其他端口...可以先修改 V2Ray 的传输协议..之后再修改 V2Ray 端口"
		echo
		change_v2ray_transport_ask
...
tls_config() {
	while :; do
		...
		echo
		echo
		echo -e "请输入一个 $magenta正确的$none域名"
	...
	while :; do
		...
		if [[ -z "$record" ]]; then
			...
			if [[ "$record" == [Yy] ]]; then
				...
				echo
				echo
				echo -e "$yellow 域名解析 = ${cyan}已经解析$none"
				echo "----------------------------------------------------------------"
				echo
				break
			else
...
path_config() {
	...
	while :; do
		...
		*[/$]*)
			echo
			echo -e " 分流路径不能包含$red / $none或$red $ $none这两个符号 "
			echo
			error
			;;
		*)
...
change_proxy_site_config() {
	if [[ $v2ray_transport == [45] ]] && [[ $caddy && $is_path ]]; then
		...
		echo " 修改成功..."
		echo
		echo -e " 打开你的域名 ${cyan}https://${domain}$none 检查一下"
		echo
		echo
	elif [[ $v2ray_transport == [45] ]] && [[ $caddy ]]; then
		path_config_ask
		...
		if [[ -z $_opt ]]; then
			...
			1)
				if [[ $ban_ad ]]; then
					...
					echo
					echo
					echo -e "$green 广告拦截已开启...如果出现异常..那就关闭它$none"
					echo
				fi
				break
				;;
			2)
				if [[ $ban_ad ]]; then
					...
					echo
					echo
					echo -e "$red 广告拦截已关闭...可以随时重新开启 $none"
					echo
				else
					echo
					echo -e " 当前广告拦截状态: $_info"
...
custom_uuid() {
	...
	while :; do
		...
		*)
			...
			echo
			echo -e " 请使用正确的 UUID 格式...使用$cyan v2ray reuuid$none 复活"
			echo "----------------------------------------------------------------"
			echo
			pause
			uuid=$myuuid
...
get_v2ray_config() {
	...
	echo
	while :; do
		read -p "$(echo -e "推荐使用 Xshell [${magenta}Y$none]:")" is_xshell
		if [[ -z $is_xshell ]]; then
			error
		else
